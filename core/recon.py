...
class ReconEngine:
    ...
    def stop(self):
        ...
        self.on_log(f"[*] Restoring {self.interface} to managed mode...")
        subprocess.run(["ip", "link", "set", self.interface, "down"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["iw", "dev", self.interface, "set", "type", "managed"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["ip", "link", "set", self.interface, "up"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["systemctl", "start", "NetworkManager", "wpa_supplicant"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        self.on_log("[!] Recon stopped and network restored.")
                ...
                for line in lines:
                    ...
                    if len(parts) >= 14 and re.match(r'^[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:', parts[0]):
                        ...
                        if bssid not in self.networks_db:
                            self.networks_db[bssid] = {"bssid": bssid, "essid": essid, "channel": channel, "signal_dbm": signal, "wps_status": "Unknown"}
        ...
        while not self.stop_event.is_set():
            ...
            if rlist:
                try:
                    ...
                    break
            if self.wash_process.poll() is not None: break
    ...
    def _process_wash_line(self, line):
        ...
        if len(parts) >= 6 and re.match(r'^[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:', parts[0]):
            ...
            if bssid in self.networks_db:
                ...
                self.networks_db[bssid] = {"bssid": bssid, "essid": " ".join(parts[6:]) if len(parts) > 6 else "Hidden", "channel": parts[1], "signal_dbm": parts[2], "wps_status": wps_status}
            self.on_update(self.networks_db)
